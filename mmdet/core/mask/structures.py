...
class PolygonMasks(BaseInstanceMasks):
    ...
    def crop_icdar(self, bbox):
        ...
        if len(self.masks) == 0:
            ...
            patch = np.array([[x1, y1], [x2, y1], [x2, y2], [x1, y2]])
            cropped_masks = []
            for poly_per_obj in self.masks:
                cropped_poly_per_obj = []
                for p in poly_per_obj:
                    ...
                    p = p.copy()
                    pc.Clear()
                    pc.AddPath(patch, pyclipper.PT_CLIP, True)
                    pc.AddPath(p.reshape(-1, 2), pyclipper.PT_SUBJECT, True)
                    p = np.asarray(pc.Execute(
                        pyclipper.CT_INTERSECTION,
                        pyclipper.PFT_EVENODD,
                        pyclipper.PFT_EVENODD))
                    if p.size >= 3 * 2:
                        p[0] -= patch[0].reshape(-1, 2)
                    else:
                        p = np.zeros([0, 3, 2])
                    cropped_poly_per_obj.append(p.reshape(-1).astype(np.float32))
                cropped_masks.append(cropped_poly_per_obj)
            cropped_masks = PolygonMasks(cropped_masks, h, w)
        return cropped_masks

    def rotate_icdar(self, rot_info):
        """Rotate masks

        Args:
            rot_info (dict): the rotation information

        Returns:
            PolygonMasks: the rotated masks
        """
        h, w, nH, nW, M, cos, sin = [
            rot_info[k] for k in ['height', 'width', 'nH', 'nW', 'M', 'cos', 'sin']
        ]
        if len(self.masks) == 0:
            rotated_masks = PolygonMasks([], nH, nW)
        else:
            rotated_masks = []
            for poly_per_obj in self.masks:
                rotated_poly_per_obj = []
                for p in poly_per_obj:
                    p = p.copy().reshape(-1, 2)  # (n, 2)

                    origin = np.asarray([w / 2, h / 2]).reshape(-1, 2)
                    translated = p - origin
                    p[:, 0] = translated[:, 0] * cos - translated[:, 1] * sin + nW / 2
                    p[:, 1] = translated[:, 0] * sin + translated[:, 1] * cos + nH / 2

                    rotated_poly_per_obj.append(p.reshape(-1).astype(np.int32))
                rotated_masks.append(rotated_poly_per_obj)
            rotated_masks = PolygonMasks(rotated_masks, nH, nW)
        return rotated_masks

    def poly2bbox(self, return_valid_inds=False):
        valid_inds = np.zeros(len(self.masks), dtype=np.bool)
        bboxes = []
        for i, poly_per_obj in enumerate(self.masks):
            x1, y1, x2, y2 = self.width, self.height, 0, 0
            for p in poly_per_obj:
                if p.shape[0] == 0 and p.size < 6:
                    continue
                valid_inds[i] = True
                xs = p[0::2]
                ys = p[1::2]
                x1 = xs.min() if xs.min() < x1 else x1
                x2 = xs.max() if xs.max() > x2 else x2
                y1 = ys.min() if ys.min() < y1 else y1
                y2 = ys.max() if ys.max() > y2 else y2
