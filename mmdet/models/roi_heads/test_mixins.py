...
class MaskPolyTestMixin(object):

    def simple_test_mask_poly(self,
                              x,
                              img_metas,
                              det_bboxes,
                              det_labels,
        ...
        if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
            segm_results = [[[] for _ in range(self.mask_head.num_classes)]
                            for _ in range(len(det_bboxes))]
            keeps = [[[] for _ in range(self.mask_head.num_classes)]
                            for _ in range(len(det_bboxes))]
            return segm_results, keeps
        ...
        for i in range(batch_size):
            ...
            if det_label.shape[0] == 0:
                ...
                segm_result, keep = self.mask_head.get_seg_masks(
                    mask_pred, det_bbox, det_label, det_socre, self.test_cfg,
                    ori_shapes[i], scale_factors[i], rescale)
                segm_results.append(segm_result)
                keeps.append(keep)
        return segm_results, keeps

    def aug_test_mask_poly(self, feats, img_metas, det_bboxes, det_labels):
        pass