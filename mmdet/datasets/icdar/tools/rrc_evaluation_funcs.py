...
def main_evaluation(p, default_evaluation_params_fn, validate_data_fn, evaluate_method_fn, show_result=True,
    ...
    if (p == None):
        l = [s[1:].split('=') for s in sys.argv[1:]]
        l[0][0] = l[0][0].replace("\x80\x93", '')
        l[1][0] = l[1][0].replace("\x80\x93", '')
        p = dict(l)
        if (len(sys.argv) < 3):
            print_help()
    ...
    #     sys.stdout.write("recall:    {}\nprecision: {}\nf-measure: {}\n".format(resDict['method']["recall"], resDict['method']["precision"], resDict['method']["hmean"]))
