...
@PIPELINES.register_module()
class ICDARRandomCrop(MinIoURandomCrop):
    """Random crop the image & bboxes & masks based on MinIoURandomCrop
        ...
        while True:
            ...
            for i in range(50):
                ...
                if len(overlaps) > 0:
                    ...
                    if 'gt_masks' in results:
                        masks_tmp = results['gt_masks'][
                            mask.nonzero()[0]].crop_icdar(patch)
                        bboxes_tmp, mask_valid_inds = masks_tmp.poly2bbox(return_valid_inds=True)
                        if not mask_valid_inds.any():
                            continue
                        results['gt_labels'] = results['gt_labels'][mask_valid_inds]
                        results['gt_masks'] = masks_tmp[mask_valid_inds.nonzero()[0]]
                        results['gt_bboxes'] = bboxes_tmp
                ...
                if 'gt_bboxes_ignore' in results:
                    ...
                    overlaps_ignore = bbox_overlaps(
                        patch.reshape(-1, 4), boxes_ignore.reshape(-1, 4)).reshape(-1)
                    mask_ignore = overlaps_ignore > 0
                    boxes_ignore = boxes_ignore[mask_ignore]
                    if len(boxes_ignore) == 0:
                        boxes_ignore = np.zeros([0, 4])
                    boxes_ignore[:, 2:] = boxes_ignore[:, 2:].clip(max=patch[2:])
                    boxes_ignore[:, :2] = boxes_ignore[:, :2].clip(min=patch[:2])
                    boxes_ignore -= np.tile(patch[:2], 2)
                    results['gt_bboxes_ignore'] = boxes_ignore

                    if 'gt_masks_ignore' in results:
                        if len(mask_ignore.nonzero()[0]) > 0:
                            masks_ignore_tmp = results['gt_masks_ignore'][
                                mask_ignore.nonzero()[0]].crop_icdar(patch)
                            results['gt_bboxes_ignore'], mask_valid_inds = masks_ignore_tmp.poly2bbox(
                                return_valid_inds=True)
                            if (mask_valid_inds != 0).any():
                                results['gt_masks_ignore'] = masks_ignore_tmp[mask_valid_inds.nonzero()[0]]
                        else:
                            masks_ignore_tmp = results['gt_masks_ignore']
                            masks_ignore_tmp.masks = []
...
@PIPELINES.register_module()
class ICDARRandomRotate(object):
    """Random rotate the image & bboxes & masks.

    Args:
        angle_range (tuple): the range of angle
    """

    def __init__(self, angle_range=(-10, 10)):
        self.angle_range = angle_range

    def get_rot_info(self, angle, img_shape):
        ...
        return dict(height=h, width=w, nH=nH, nW=nW, M=M, cos=cos, sin=sin)
    ...
    def __call__(self, results):
        ...
        # random angle
        angle = random.uniform(min(self.angle_range), max(self.angle_range))

        # get rotation information
        rot_info = self.get_rot_info(angle, results['img'].shape[:2])

        # rotate image
        results['img'] = cv2.warpAffine(results['img'], rot_info['M'],
                                        (rot_info['nW'], rot_info['nH']),
                                        flags=cv2.INTER_LINEAR)
        results['img_shape'] = results['img'].shape

        # rotate bboxes & masks
        if 'gt_bboxes' in results and 'gt_masks' in results:
            results['gt_masks'] = results['gt_masks'].rotate_icdar(rot_info)
            results['gt_bboxes'] = results['gt_masks'].poly2bbox(return_valid_inds=False)

        if 'gt_bboxes_ignore' in results and 'gt_masks_ignore' in results:
            results['gt_masks_ignore'] = results['gt_masks_ignore'].rotate_icdar(rot_info)
            results['gt_bboxes_ignore'] = results['gt_masks_ignore'].poly2bbox(return_valid_inds=False)
